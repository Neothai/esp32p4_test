...
typedef struct {
    ...
    avi_off_t movi_first_start;  /* first OpenDML block (never overwritten) */
    avi_off_t movi_first_end;

    /* OpenDML block table (optional, registered by the caller) */
    avi_off_t mb_start[AVI_MAX_MOVI_BLOCKS];
    avi_off_t mb_end[AVI_MAX_MOVI_BLOCKS];
    int       mb_cnt, mb_cur;
    ...
    avi_off_t retry_pos;      /* [เพิ่ม] จุดเริ่ม chunk ที่อ่านไม่สำเร็จ (0 = ไม่มี) */
    int       retry_is_video; /* [เพิ่ม] chunk ที่ค้างไว้เป็นวิดีโอหรือไม่ */
...
/* Point the streaming reader at an explicit 'movi' region. Use this with an
 * OpenDML block map instead of relying on the reader to find the next block. */
void AVI_set_movi_region(avi_t *AVI, avi_off_t start, avi_off_t end);

/* Register one OpenDML 'movi' block (call once per block, in file order,
 * before the first read). AVI_read_data() then switches blocks by itself and
 * AVI_seek_start() rewinds to the first block. */
...
avi_off_t   AVI_audio_bytes(avi_t *AVI);

/* [เพิ่ม] อ่าน chunk ที่เพิ่งล้มเหลวเพราะ "บัฟเฟอร์เล็กเกินไป" ซ้ำอีกครั้ง
 * ใช้คู่กับ AVI_read_data() ที่คืน -1 (วิดีโอ) หรือ -2 (เสียง):
 *   ret = AVI_read_data(a, vbuf, vcap, abuf, acap, &len);
 *   if (ret < 0) { ขยายบัฟเฟอร์เป็น len + margin; AVI_retry_last_chunk(a); }
 * คืน 0 = ตั้งตำแหน่งถอยหลังสำเร็จ, -1 = ไม่มี chunk ที่รอ retry
 * (บัฟเฟอร์ไม่ถูกแตะเลย lib ไม่ realloc ให้ เพราะบัฟเฟอร์เป็นของผู้เรียก) */
