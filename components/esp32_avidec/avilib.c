...
/* 8 + padded size, guaranteed to advance (no 32-bit wrap) */
static inline avi_off_t chunk_span(uint32_t sz)
{
    avi_off_t s = (avi_off_t)8 + (avi_off_t)pad2(sz);
    return s < 8 ? 8 : s;
}
...
static bool read_at(avi_t *avi, avi_off_t off, void *dst, uint32_t len)
{
    ...
    /* อยู่นอกหน้าต่างทั้งหมด: คัดลอกส่วนที่ค้างในหน้าต่างก่อน แล้วอ่านเฉพาะ
       ส่วนที่เหลือ (ไม่ทำให้ FIL pointer กับ buf_off+buf_len หลุดกัน) */
    if (off >= avi->buf_off && off < avi->buf_off + avi->buf_len) {
        uint32_t n = (uint32_t)(avi->buf_off + avi->buf_len - off);
        UINT rd = 0;
        memcpy(dst, avi->buf + (off - avi->buf_off), n);
        if (n == len) {
            return true;
        }
        if (f_lseek(&avi->fil, (FSIZE_t)(off + n)) != FR_OK)
            return false;
        if (f_read(&avi->fil, (uint8_t *)dst + n, len - n, &rd) != FR_OK || rd != len - n)
            return false;
        avi->buf_off = off + len;
        avi->buf_len = 0;
        return true;
    ...
    /* นอกหน้าต่างสนิท: อ่านตรง 1 ครั้ง แล้วตั้งหน้าต่างใหม่ให้ตรงกับตำแหน่งไฟล์ */
    {
        UINT rd = 0;
        ...
        if (f_read(&avi->fil, dst, len, &rd) != FR_OK || rd != len)
            return false;
        avi->buf_off = off + len;      /* FIL ptr == buf_off + buf_len */
        avi->buf_len = 0;
    }
    return true;
}
...
/* ------------------------------------------------------------------ */
/* resync: ค้นหา signature ของบล็อกถัดไป (ใช้เมื่อขนาด chunk ที่อ่านได้
   เป็นขยะ/มี gap คั่น เพื่อไม่ให้ตัวอ่านยอมแพ้เป็น EOF กลางไฟล์)        */
/* ------------------------------------------------------------------ */
static avi_off_t avilib_resync(avi_t *avi, avi_off_t from, uint32_t window)
{
    uint8_t buf[4096 + 16];
    const uint32_t step = 2;              /* chunk ใน AVI อยู่ในแนว 2 ไบต์ */
    avi_off_t end = from + window;
    if (end > avi->file_size) end = avi->file_size;

    for (avi_off_t p = from; p + 12 <= end; ) {
        uint32_t n = sizeof(buf);
        if ((avi_off_t)n > end - p) n = (uint32_t)(end - p);
        if (!read_at(avi, p, buf, n)) break;

        for (uint32_t i = 0; i + 12 <= n; i += step) {
            if (rd32(buf + i) == TAG_LIST) {
                if (rd32(buf + i + 8) == TAG_movi)
                    return p + i;
            } else if (rd32(buf + i) == TAG_RIFF) {
                if (rd32(buf + i + 8) == FCC4('A','V','I','X'))
                    return p + i;
            }
        }
        if (n <= 12) break;
...
static int build_index_scan(avi_t *a)
{
    ...
    /* สแกนทุกบล็อกของ OpenDML ไม่ใช่แค่บล็อกแรก (ไม่งั้น index ขาดช่วง) */
    int nblk = (a->mb_cnt > 0) ? a->mb_cnt : 1;
    for (blk = 0; blk < nblk; blk++) {
        if (a->mb_cnt > 0) {
            a->movi_start = a->mb_start[blk];
            a->movi_end   = a->mb_end[blk];
            a->mb_cur     = blk;
        }
        a->pos = a->movi_start;
    ...
    for (;;) {
        ...
        if (a->pos + 8 > a->movi_end) {
            /* the LIST size may understate the real data (see AVI_read_data) */
            uint32_t cid2, csz2;
            if (peek_hdr(a, &cid2, &csz2) && is_video_chunk(a, cid2) &&
                a->pos + chunk_span(csz2) <= a->file_size) {
                a->movi_end = a->pos + chunk_span(csz2);
                continue;
            }
            break;
        }
    ...
    if (a->mb_cnt > 0) {          /* restore the first block as the cursor */
        a->movi_start = a->mb_start[0];
        a->movi_end   = a->mb_end[0];
        a->mb_cur     = 0;
    }
...
/* Switch the streaming reader to an explicit movi region (OpenDML block).
 * The caller supplies the region (e.g. from its own block map) so the reader
 * never has to *guess* where a block ends. */
/* Register the OpenDML 'movi' blocks once (start = first chunk, end = one
 * past the last chunk). AVI_read_data() then walks them by itself. */
int AVI_add_movi_block(avi_t *AVI, avi_off_t start, avi_off_t end)
{
    int i;
    if (!AVI || start == 0 || end <= start)
        return -1;
    for (i = 0; i < AVI->mb_cnt; i++)
        if (AVI->mb_start[i] == start)      /* already registered */
            return 0;
    if (AVI->mb_cnt >= AVI_MAX_MOVI_BLOCKS) {
        SETERR(AVI_ERR_SIZELIM);
        return -1;
    }
    AVI->mb_start[AVI->mb_cnt] = start;
    AVI->mb_end[AVI->mb_cnt] = end;
    AVI->mb_cnt++;
    return 0;
}

void AVI_set_movi_region(avi_t *AVI, avi_off_t start, avi_off_t end)
{
    int i;
    if (!AVI)
        return;
    AVI->mb_cur = -1;                     /* re-sync the block-table cursor */
    for (i = 0; i < AVI->mb_cnt; i++)
        if (AVI->mb_start[i] == start) { AVI->mb_cur = i; break; }
    AVI->movi_start = start;
    AVI->movi_end = end;
    AVI->pos = start;
    AVI->buf_off = start;
    AVI->buf_len = 0;
    f_lseek(&AVI->fil, (FSIZE_t)start);   /* keep FIL_ptr == buf_off + buf_len */
}
...
int AVI_parse(avi_t *AVI, int getIndex)
{
    ...
    AVI->movi_first_start = AVI->movi_first_end = 0;
    AVI->idx1_off = 0;
    AVI->idx1_len = 0;
    AVI->anum = 0;
    AVI->aptr = 0;
    ...
    while (pos + 8 <= AVI->file_size) {
        ...
        if (id == TAG_LIST) {
            ...
            if (ltype == TAG_hdrl)
                ...
                avi_off_t ms = pos + 12;
                avi_off_t me = pos + 8 + sz;
                if (me > AVI->file_size)
                    me = AVI->file_size;
                if (AVI->movi_start == 0) {          /* first block wins */
                    AVI->movi_start = ms;
                    AVI->movi_end = me;
                    AVI->movi_first_start = ms;
                    AVI->movi_first_end = me;
                    AVI->mb_cur = 0;
                }
                AVI_add_movi_block(AVI, ms, me);     /* collect all blocks */
            }
            pos += 8 + pad2(sz);
            continue;
        }
        ...
        /* OpenDML: 'RIFF' <size> 'AVIX' -> เดินเข้าไปข้างในเพื่อเก็บ LIST movi
           และ idx1 ของก้อนขยาย (เดิมข้ามทั้งก้อน ทำให้ idx1 ไม่เคยถูกพบ และ
           index ครอบแค่บล็อกแรกของไฟล์ >1GB) */
        if (id == TAG_RIFF) {
            uint8_t t[4];
            if (read_at(AVI, pos + 8, t, 4) &&
                (rd32(t) == FCC4('A','V','I','X') || rd32(t) == TAG_AVI)) {
                pos += 12;
                continue;
            }
        }
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes)
{
    ...
    if (t->audio_index == NULL || t->audio_chunks <= 0)
        return 0;                       /* ไฟล์นี้ไม่มีแทร็กเสียง (เดิม NULL deref) */
    if (t->audio_posc < 0)
        t->audio_posc = 0;
    if (t->audio_index == NULL || t->audio_chunks <= 0)
        return 0;
    if (t->audio_posc >= t->audio_chunks)
        return 0;
    if ((avi_off_t)t->audio_posb > t->audio_index[t->audio_posc].len)
        return 0;                       /* ตำแหน่งค้าง/เกินขนาด chunk -> กัน left wrap */
                              ...
                              char *audbuf, long max_audbuf, long *len)
{
    ...
    for (;;) {
        ...
        if (AVI->pos + 8 > AVI->movi_end) {
            ...
            /* (1) ★ ตรวจก่อนเป็นอันดับแรก: LIST size ของก้อนปัจจุบันอาจ "สั้นกว่าความจริง" (muxer เขียน
                   size ผิด/ตัดที่เพดาน) ถ้าที่ movi_end ยังเป็น chunk ของวิดีโอ/
                   เสียงที่สมบูรณ์ ให้ต่อขอบเขตออกไปอ่านต่อ ไม่งั้นเฟรมจะหายเงียบ ๆ */
            {
                uint32_t cid2, csz2;
                if (peek_hdr(AVI, &cid2, &csz2)) {
                    bool is_av = is_video_chunk(AVI, cid2);
                    int tt;
                    for (tt = 0; !is_av && tt < AVI->anum; tt++) {
                        uint32_t tag = FCC4((uint8_t)AVI->track[tt].audio_tag[0],
                                            (uint8_t)AVI->track[tt].audio_tag[1],
                                            (uint8_t)AVI->track[tt].audio_tag[2],
                                            (uint8_t)AVI->track[tt].audio_tag[3]);
                        if (cid2 == tag) is_av = true;
                    }
                    if (is_av && AVI->pos + chunk_span(csz2) <= AVI->file_size) {
                        AVI->movi_end = AVI->pos + chunk_span(csz2);
                        continue;
                    }
                }
            }

            /* (0) ถ้าข้อมูลตรงหน้าไม่ใช่ chunk ของ stream แล้วจริง ๆ จึงกระโดด
                   ไปบล็อกถัดไปตามตาราง (OpenDML ที่ LIST size ไม่ตรง / มี gap คั่น) */
            if (AVI->mb_cur >= 0 && AVI->mb_cur + 1 < AVI->mb_cnt) {
                AVI->mb_cur++;
                AVI->movi_start = AVI->mb_start[AVI->mb_cur];
                AVI->movi_end   = AVI->mb_end[AVI->mb_cur];
                AVI->pos        = AVI->movi_start;
                continue;
            }
            ...
            while (AVI->pos + 8 <= AVI->file_size) {
                ...
                if (cid == TAG_RIFF) {
                    ...
                    if (read_at(AVI, AVI->pos + 8, t, 4) && rd32(t) == TAG_movi) {
                        ...
                        if (AVI->mb_cnt > 0) {   /* keep the table cursor in sync */
                            int bi;
                            for (bi = 0; bi < AVI->mb_cnt; bi++)
                                if (AVI->mb_start[bi] == AVI->movi_start) { AVI->mb_cur = bi; break; }
                        }
                ...
                // แต่ถ้าขนาดดูไม่น่าเชื่อถือ (pos ไม่ขยับ / กระโดดเกินไฟล์)
                // ให้หยุดค้นหา เพื่อให้ผู้เรียกสลับไปก้อนถัดไปของตัวเอง
                {
                    avi_off_t span = chunk_span(csz);
                    if (span >= 8 && AVI->pos + span <= AVI->file_size) {
                        AVI->pos += span;
                        continue;
                    }
                }

                /* ขนาดที่อ่านได้เป็นขยะ (มี gap/JUNK คั่น หรือ muxer เขียนผิด):
                   ค้นหา signature 'RIFF'/'LIST' ในหน้าต่างถัดไปแบบมีขอบเขต
                   แล้วกู้การเล่นต่อ ดีกว่ายอมแพ้เป็น EOF กลางไฟล์ (ค้าง) */
                {
                    avi_off_t found = avilib_resync(AVI, AVI->pos + 2, 1u << 20);   /* คงแนว 2 ไบต์ */
                    if (found) {
                        AVI->pos = found;
                        AVI->retry_pos = 0;
                        continue;
        ...
        if (is_video_chunk(AVI, id)) {
            ...
            AVI->video_pos++;
            if (sz > (uint32_t)max_vidbuf) {
                /* [เพิ่ม] จำตำแหน่ง chunk นี้ไว้ ให้ผู้เรียกขยายบัฟเฟอร์แล้วอ่านซ้ำได้ */
                AVI->retry_pos = payload - 8;
                AVI->retry_is_video = 1;
                return -1;
            }
            if (!read_at(AVI, payload, vidbuf, sz))
                return 0;
...
/* seeking                                                            */
/* ------------------------------------------------------------------ */
/* [เพิ่ม] อ่าน chunk ที่เพิ่งล้มเหลวเพราะบัฟเฟอร์เล็กเกินไป ซ้ำอีกครั้ง
 * (ดูคอมเมนต์ใน avilib.h) — ตั้งตำแหน่งถอยกลับไปที่หัว chunk เดิม
 * และถอยตัวนับ video_pos เพราะ chunk นั้นยังไม่ถูกส่งออกไปจริง      */
int AVI_retry_last_chunk(avi_t *AVI)
{
    if (!AVI || AVI->retry_pos == 0)
        return -1;
    AVI->pos = AVI->retry_pos;
    if (AVI->retry_is_video && AVI->video_pos > 0)
        AVI->video_pos--;
    AVI->retry_pos = 0;
    return 0;
}
...
int AVI_seek_start(avi_t *AVI)
{
    int t;
    AVI->retry_pos = 0;
    /* rewind to the FIRST movi block, not to whatever block the streaming
     * reader happened to move to last (OpenDML) */
    if (AVI->mb_cnt > 0) {
        AVI->mb_cur = 0;
        AVI->movi_start = AVI->mb_start[0];
        AVI->movi_end = AVI->mb_end[0];
    } else if (AVI->movi_first_start) {
        AVI->movi_start = AVI->movi_first_start;
        AVI->movi_end = AVI->movi_first_end;
    }
    AVI->pos = AVI->movi_start;
    AVI->video_pos = 0;
    for (t = 0; t < AVI_MAX_TRACKS; t++) {
...
int AVI_set_audio_position(avi_t *AVI, avi_off_t byte)
{
    ...
    if (t->audio_posb < 0)
        t->audio_posb = 0;
    if ((avi_off_t)t->audio_posb > t->audio_index[n0].len)
        t->audio_posb = t->audio_index[n0].len;   /* clamp กัน left wrap */
    return 0;
}
